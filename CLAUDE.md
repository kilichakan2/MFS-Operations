...
- `npm run test:e2e:preview -- <preview-url> --unprotected` — Gate-4 preview smoke: the full `@critical` suite (75 specs as of F-20 PR2) against a PR's deployed Vercel preview wired to its Supabase preview branch (remote-only, fail-closed; see `docs/runbooks/preview-smoke.md`). **You MUST pass `--unprotected`** — Vercel Deployment Protection is OFF (F-INFRA-02), so no `VERCEL_AUTOMATION_BYPASS_SECRET` is needed or available; WITHOUT the flag the script fails closed with "bypass secret missing" and exits 0 (a silent false-green — BACKLOG F-INFRA-06). **Readiness gate before running:** poll `https://<preview-url>/api/auth/team` until it returns **200**, NOT `/login` — `/login` returns 200 even before the F-INFRA-05 cred-sync redeploy wires the Supabase creds, and the DB-identity probe (check 2) 500s if you run against the pre-cred-sync deploy.
- `npm run db:branches` — list Supabase preview branches (ship-checklist "no orphaned branches" check)

**Migration filenames must use a full 14-digit timestamp: `YYYYMMDDHHMMSS_name.sql`** (e.g. `20260613020000_harden_security_definer_fns.sql`). The older `YYYYMMDD_NNN` form is banned — the Supabase CLI reads the version from the digits before the first underscore, so two same-day migrations collide (`schema_migrations_pkey` 23505 on `db:reset`) and the short form breaks Supabase preview-branch resync (branch `status=MIGRATIONS_FAILED` on a PR's 2nd+ push). Enforced by `tests/unit/migrations/filename-convention.test.ts`.
