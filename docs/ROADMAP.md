...
> **In flight (parallel terminal):** UI-system rebuild on `worktree-ui-system-rebuild` — clear to merge now the sprint is sealed (gated by the live `smoke` required check). Tracker: [`docs/plans/2026-06-28-ui-overhaul-roadmap.md`](plans/2026-06-28-ui-overhaul-roadmap.md).
> The table below is the PRE-SPRINT roadmap (hardware / printing / order-pipeline items); items 4–6 fold into F-PROD-04. Treat the sprint docs above as current; this table is being reconciled.
