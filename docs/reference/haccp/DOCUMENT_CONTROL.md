...
**Exception — CCP-M Mince & Meat Prep limits are also configurable in the app.** Since Jul 2026 the mince/meat-prep production limits (6 temperature channels: mince/prep input, mince/prep output chilled, mince/prep output frozen) AND the per-species kill-day limits are stored in the database (`haccp_mince_thresholds`) and editable by an **admin** at `/haccp/admin` → Thresholds. Every change is recorded in an immutable audit log (`haccp_mince_threshold_audit`: who / when / old→new) and the admin is reminded on save to update THIS register (section 4) and retrain staff. The band STRUCTURE is fixed in code — only the numbers move; kill-day rows are structurally amber-less (binary), and the imported/vac-packed "no kill-day limit" row cannot be given a limit via the app (see justification (3) below). **IMPORTANT — CCP-M amber bands are DISPLAY-ONLY:** unlike CCP 1's conditional-accept amber, a CCP-M reading in the warning band still requires AND files a corrective action — the amber colour changes the screen, never the paperwork. Corrective action triggers on anything above the pass limit, exactly as before the bands existed.

CCP 1 and CCP 3 use a three-band model: **pass** (≤ target), **amber / conditional accept** (above target, up to the ceiling — corrective action logged), **reject / critical** (above the ceiling — corrective action logged AND management sign-off required). CCP-M uses a three-COLOUR model with two-band paperwork: **pass** (≤ pass limit), **warning** (display-only colour up to the warning ceiling — corrective action still required and filed), **deviation** (above the ceiling — same corrective action path).
...
| CCP-M1 / CCP-MP1 — Mince & Prep input | Pass ≤7°C · warning 7–8°C (display only, CA still filed) · deviation >8°C | ≤7°C | Reg 853/2004 Annex III Sec V Ch III (7°C red-meat working limit); see justification (4) |
| CCP-M1 — Mince output (chilled) | Pass ≤2°C · warning 2–3°C (display only, CA still filed) · deviation >3°C | ≤2°C | Reg 853/2004 Annex III Sec V Ch III (minced meat ≤2°C); see justification (5) |
| CCP-MP1 — Prep output (chilled) | Pass ≤4°C · warning 4–5°C (display only, CA still filed) · deviation >5°C | ≤4°C | Reg 853/2004 Annex III Sec V Ch III (meat preparations ≤4°C); see justification (6) |
| CCP-M1 / CCP-MP1 — Output (frozen) | Pass ≤-18°C · warning -18 to -17°C (display only, CA still filed) · deviation >-17°C | ≤-18°C | Reg 853/2004 (≤-18°C frozen); see justification (7) |
| CCP-M2 — Kill days (lamb / beef fresh) | ≤6 days from kill (binary — over = hard block, DO NOT MINCE) | ≤6 days | Reg 853/2004 Annex III Sec V Ch III pt 2(b) |
| CCP-M2 — Kill days (imported / vac-packed) | No app-enforced kill-day limit — recorded for traceability only | — | See justification (3) below |
...
**Written justifications for the bands that deviate from the strictest legal reading:**

1. **Red meat reject line at >8°C (CCP 1)** — Regulation 853/2004's specific red-meat transport limit is 7°C core; this register's reject line sits 1°C above it. Retained per the general UK chill-holding requirement of 8°C, with the 5–8°C band treated as a conditional accept with mandatory corrective action (place into coldest chiller, halve remaining shelf life, document, review supplier). Decision: Hakan, 2026-07-02.
2. **Poultry 4–5°C grace band (CCP 1)** — the legal limit is ≤4°C (Reg 853/2004 Annex III Sec II) with no legal headroom. The 4–5°C band is a documented 1°C grace band for probe/unloading measurement fluctuation, treated as amber with mandatory corrective action; >5°C is a hard reject. Decision: Hakan, 2026-07-02.
3. **Imported / vac-packed — no app-enforced kill-day limit (CCP-M2)** — Reg 853/2004 Annex III Sec V Ch III pt 2(b)(iii) permits minced meat to be produced from "boned, vacuum-packed beef and veal, no more than 15 days after the slaughter of the animals" (verified 2026-07-02). The app consciously does NOT enforce a 15-day clock for the `imported_vac` species: the kill-date remains a recorded, operator-controlled manual check (supplier documentation + intake verification), because imported vac-packed consignments routinely carry pack dates rather than reliable kill dates, and a wrong in-app clock would either false-block legal product or false-pass illegal product. The kill date is still captured on every run for traceability. Frozen-route nuance: no day-clock applies where the meat was boned before freezing (pt 2(b)(iv) route), and re-freezing after thawing is prohibited (pt 5). Decision: Hakan, 2026-07-02.
4. **Mince/prep input 7–8°C warning band (CCP-M1/MP1)** — the working limit is ≤7°C (Reg 853/2004 Annex III Sec V Ch III red-meat raw material). The 7–8°C band is a 1°C **display-only** grace for probe/handling measurement fluctuation during active production: the reading shows a warning colour instead of a hard-fail colour, but the record persists as a failed check and the corrective action is required and filed exactly as for any reading above 7°C. Decision: Hakan, 2026-07-02.
5. **Mince output 2–3°C warning band (CCP-M1)** — the legal storage limit for minced meat is ≤2°C (same Chapter). The 2–3°C band is a 1°C **display-only** grace for probe/handling fluctuation immediately post-chill; corrective-action paperwork is unchanged (fires above 2°C). Decision: Hakan, 2026-07-02.
6. **Prep output 4–5°C warning band (CCP-MP1)** — the legal storage limit for meat preparations is ≤4°C (same Chapter). The 4–5°C band is a 1°C **display-only** grace; corrective-action paperwork is unchanged (fires above 4°C). Decision: Hakan, 2026-07-02.
7. **Frozen output -18 to -17°C warning band (CCP-M1/MP1)** — the frozen limit is ≤-18°C. The -18 to -17°C band is a 1°C **display-only** grace for probe fluctuation on freshly frozen product; corrective-action paperwork is unchanged (fires above -18°C). Decision: Hakan, 2026-07-02.
...
| Jul 2026 | Critical Temperature Limits (section 4) | CCP-M Mince & Meat Prep rows added: 6 temperature channels (input ≤7°C, mince output chilled ≤2°C, prep output chilled ≤4°C, frozen outputs ≤-18°C — each with a 1°C DISPLAY-ONLY warning band, justifications (4)–(7)) + per-species kill-day limits (lamb/beef ≤6 days binary hard block; imported/vac-packed no app-enforced limit — vac-pack deviation, justification (3)). Limits now admin-configurable in the app (`haccp_mince_thresholds`, immutable `haccp_mince_threshold_audit`) like CCP 1/CCP 3 — this register remains the approved source of truth. NOTE: CCP-M warning bands change tile colours only — corrective action still triggers and files on anything above the pass limit (unlike CCP 1's conditional-accept amber). Staff to be retrained on the warning-colour meaning. | — | Hakan Kilic |
