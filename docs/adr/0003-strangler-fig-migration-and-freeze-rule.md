...
**FREEZE rule.** Once F-04 (the ESLint guard that forbids `from "@supabase/supabase-js"` outside `lib/adapters/supabase/**`) ships in Phase 0, no new code may import the Supabase SDK outside the adapter folder. The shared service-role client lives at `lib/adapters/supabase/client.ts` (relocated from the former `lib/supabase.ts` by F-TD-04, which also made its construction lazy), fully inside the allow-listed adapter folder. The lint guard freezes the existing surface area at its current 88-route footprint so the migration can drain it without anyone backfilling. New routes added during the migration period either go straight through the new layer or stop. F-04 lands as part of the Phase 0 work (architecture review line 310); F-27 in Phase 5 (line 360) tightens the same rule to cover every vendor SDK on the list, at which point the rip-out test becomes a CI-enforced guarantee rather than a review discipline.
