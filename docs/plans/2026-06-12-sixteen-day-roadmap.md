...
- **F-RLS-04a** — Orders-context RLS migration (expand-contract, rollback path)
  ...
  - ✅ **F-INFRA-05 shipped (2026-06-15) → preview cred-sync unblocked:** the Supabase→Vercel Preview cred-sync now runs automatically, so the Gate-4 preview smoke has working DB creds. F-RLS-04a is NOT inert (flips real Orders routes) and genuinely needs that smoke — now available.
  - ⚠️ **Still open — `SUPABASE_JWT_SECRET` on preview:** F-INFRA-05 proved the Supabase Management API does NOT return the JWT secret for a branch (asymmetric-key migration), so the cred-sync can't supply it. F-RLS-04a must source the preview JWT secret another way (mechanism undecided) before flipping Orders routes.
