...
- **F-24** — `MapProvider` port + Leaflet adapter (separate pass — wrap the already-shipped map vendor behind an owned socket). Split into 2 PRs (Route Planner map now, admin Map View next).
  ...
  - **PR2 ⬜ NEXT** — migrate `components/MapView.tsx` (admin Screen 6) onto MapProvider; remove the F-24 PR1 fence exception. Needs the cluster surface (`leaflet.markercluster` / `react-leaflet-cluster`) handled behind the adapter.
- **F-RLS-04c** — Routes-context RLS (separate pass — flips the 5 Routes routes onto `routesServiceForCaller` + adds Routes write policies; mirror of F-RLS-04a/04b). After F-24.
