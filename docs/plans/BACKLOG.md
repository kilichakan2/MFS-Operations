...
- **▶ Owner unit (NEXT) — F-PROD-04 mince + meat-prep label print section (Hakan, 2026-06-30):** build the **native mince label** (same 52×38mm label-mode format as the goods-in/delivery label — the deferred fast-follow: add a native `printMinceLabel`/extend the `printLabel` JSON `type` to "mince" + render in `SunmiPrintBridge.java`; the JSON bridge means NO signature change needed) AND a **new label-print UI section for mince AND meat prep**. The mince/prep label = the same format PLUS process-location fields **"Minced in" / "Further cut in"** (shorten as needed, e.g. "Minced:" / "Cut:") carrying **MFS's FSA approval code** (the establishment/plant code, e.g. `UK2946` — confirm) to show MFS is the processor (food-law traceability for minced/prepped product). UI: add the print entry points on the mince + meat-prep screens (use existing components per UI decisions #17/#19 — DB-captured fields = label-only; new staff-entry fields = via existing `components/ui/`). Its own gated FORGE→ANVIL pass; on-device calibration on the V3 (the label-learning is already done on the device).
