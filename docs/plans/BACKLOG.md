...
- **Owner unit:** unscheduled — **strong candidate to pair with F-TD-30** (cash audit trail): identical mechanism (write an `audit_log` row per mutation, actor from the threaded `userId` or the `app.current_user_id` GUC), shared `screen`-enum / trigger work. Doing both in one "audit-trail standardisation" unit avoids designing the enum/trigger twice.
- **Status:** open
