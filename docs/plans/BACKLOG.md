...
- **✅ die-cut label sizing (DELIVERY) SHIPPED 2026-06-30 (PR #101 `84e886a` + calibration `816455c`):** the V3 silent native DELIVERY print now fits 52×38mm die-cut stock. Receipt mode → **label mode** (`labelLocate`/`labelOutput`); layout trimmed (no header; species+batch one row; barcode shrunk to 40 dots; **Born/Reared separate cells**; **Temp value-only**, no PASS/FAIL; keep supplier/date/sl/cut/allergens). **Bridge made VERSION-TOLERANT (ADR-0013):** new `printLabel(String json)` (parsed by name via platform `org.json`) + kept the legacy positional method (both → one `renderDeliveryLabel`); JS `buildDeliveryPayload` + feature-detect. This fixed a SILENT no-print: the app is a remote-URL shell, so the bridge JS ships from the deployed web while the Java ships in the APK — a positional arg-count change made the two desync with no error. ADR-0012 (label mode/layout) + ADR-0013 (JSON bridge) Accepted. ANVIL: unit 24/24 · JSON keys 11/11 · signed vc4/1.3 · smoke 78 (reseeded preview, F-INFRA-07 flake cleared). **Ship = publish-then-calibrate (Hakan):** merged + installed, then tuned live on the V3. **TWO key on-device learnings:** (1) the Sunmi SDK has NO software label-mode/gap-learning call — the printer must be set to **Label/Gap paper + run label-learning in the DEVICE printer settings** (one-time); only then do `labelLocate`/`labelOutput` align. (2) manual space-pad columns overflow at large fonts → use the Sunmi **`printColumnsString`** column API (widths sum to ~32 for the 58mm head). Cert `docs/anvil/2026-06-30-fprod04-diecut-label-sizing-cert.md`.
- **✅ SHIPPED 2026-06-30 (PR #102 `0a3a1cb`) — web LIVE; on-device native calibration PENDING — F-PROD-04 beef mince + meat-prep DISPATCH labels.** All render paths (HTML/ZPL/native) now beef-labelling-compliant: `MFS_PLANT_CODE='GB2946'` constant replaced hardcoded UK2946 (also corrects the delivery label); two templates — MINCE country-only (`Slaughtered in GB` / `Minced in GB`) vs PREP country+plant (`Slaughtered in GB1234` / `Cut in GB5678` / `Further cut in GB2946`); multi-source = distinct comma-join; server aggregates BLS data once, native consumes via `format=json`; native bridge gained `mince`/`prep` `type`-branches with NO signature change (ADR-0013). Guard NO BLOCKERS; ANVIL CLEARED (`docs/anvil/2026-06-30-beef-mince-prep-dispatch-labels-cert.md`): unit 3016/3016 (21 BLS oracle cases) · @critical 78/78 preview (F-INFRA-07 flake on the test-only 2nd push → reseed branch `338e7416…` + rerun SAME head → clean) · NO migration/RLS/PITR/new-dep · prod smoke 6/6 → 200. Review `docs/reviews/2026-06-30-beef-mince-prep-dispatch-labels-review.md`. **✅ FULLY DONE 2026-06-30 — MINCE + PREP both confirmed printing/fitting on the V3 (native calibrated vc7/1.6 `d2d0ce6`: dropped the bold batch line — batch stays in the barcode caption — + removed the under-barcode gap → fits 52×38).** Three follow-on web fixes shipped during on-device testing: **(1) mince 404** (PR #103 `5ed1c24`) — `/api/labels` mince select asked for `allergens_present`, a column only on `haccp_meatprep_log` → PostgREST 404 → native fell back to iframe (can't print on V3); dropped the column, render None till Pass 3. **(2) traceability chaining** (PR #104 `c78d5eb`) — prep/mince were BLANK for Born/Slaughtered/Cut when sourced from another BATCH (burger from a mince batch, `source_delivery_ids=[]`); new `resolveDeliveryIds()` follows source batches back to their deliveries (MINCE-/PREP- → their source rows; PREP recurses 1 level, depth-bounded; else a goods-in batch_number). **(3) Born/Reared as country CODES** (GB, AU) for fit + consistency (same PR; goods-in label unchanged). All verified vs prod DB + @critical 78/78. **Open backlog gaps from this session:** run vitest integration in CI (would've caught the 404; not in `smoke`); native `printLabel` catch SWALLOWS exceptions → native render errors fail SILENTLY (robustness gap); extend the integration suite with a prep-from-mince-batch chain case. **→ NEXT = Pass 3 (real allergens on delivery + mince/prep — stop hardcoding "None").** Original framing retained below for context:
- **(Framing, 2026-06-30 — now SHIPPED above) F-PROD-04 beef mince + meat-prep DISPATCH labels (Hakan):** make the dispatch (mince + prep) labels BEEF-LABELLING-COMPLIANT and print them natively on the V3 die-cut roll. **Driven by the RPA inspector's email (Daniel Hamilton, RPA Regulatory & Advice Service, 2026-06-30) — see the compulsory-requirements digest below.** Hakan: "beef labelling is the most important to get right, the rest can follow."
  - **Decisions locked 2026-06-30 (AskUserQuestion):** (1) **Plant code = `GB2946`** on the prep label as "Further cut in GB2946" (the beef-labelling-scheme prefix is GB, NOT the health-mark UK2946 — `UK 2946 EC` is the oval mark, `GB2946` is the BLS form; SAME plant number 2946). Replace the hardcoded `UK2946` (`app/api/labels/route.ts:132`, `lib/printing/types.ts:26`) with a named constant `GB2946`. (2) **Physical target = the same V3 die-cut 52×38mm native print** — build a native mince + prep renderer branch in `SunmiPrintBridge.java` via the JSON `type` key (ADR-0013: change JSON keys, NEVER the signature) + `buildMincePayload`/`buildPrepPayload` in `lib/adapters/sunmi/Printer.ts`. (3) **Scope = beef-correct mince+prep NOW; defer veal + mixed-origin-split + processed-exemption** to follow-on units (below).
  - **What ALREADY EXISTS (recon 2026-06-30, don't rebuild):** the mince+meatprep screen (`app/haccp/mince/page.tsx`, 3 sub-forms mince/meatprep/timesep) ALREADY captures source deliveries + prints a label (via `getPrinter()`/`PrintLabelStrip`); a **per-day production batch code already exists** (`HaccpDailyChecksService.ts:449-461` `buildBatchCode` → `MINCE-DDMM-SP-N` / `PREP-DDMM-SP-N`) — that IS Danny's "batch unique to one day's production" (the intake delivery batch `DDMM-CC-N` feeds in as `source_batch_numbers`). Intake (`haccp_deliveries`) already stores `born_in`/`reared_in`/`slaughter_site`/`cut_site` (plant codes). HTML/ZPL mince renderers exist (`renderMinceHTML` html.ts:222, zpl.ts:157) with "Minced in: GB".
  - **The actual gaps to build:** (a) **NATIVE mince/prep label on the V3** — the native Sunmi renderer only knows the *delivery* label; no native mince/prep renderer yet. (b) **Compulsory wording + the mince≠prep SPLIT** — today there's ONE mince template; need TWO: **MINCE** = Origin/born&reared · `Slaughtered in GB` (country) · day-batch · `Minced in GB` (country, NO plant code); **PREP** = Origin/born&reared · `Slaughtered in GB1234` (country+plant) · `Cut in GB5678` (primary cut site, from intake `cut_site`) · `Further cut in GB2946` (MFS) · day-batch. (c) Use the EXACT compulsory wording (list below) — the native delivery label's abbreviated `Born:`/`Sl:`/`Cut:` is OK for an ON-SITE label but dispatch needs full words. (d) GB2946 constant. Allergens (Pass 3) touches the same renderers.
  - **Compliance note (reassurance):** the goods-in/DELIVERY die-cut label already shipped is COMPLIANT — Danny's "beef on site" rule = minimum a batch number traceable to intake records; our delivery label carries batch+barcode+traceability. Full compulsory wording is only required on DISPATCH (mince/prep) labels.
  - UI: print entry points already on the mince/meatprep screen; per decisions #17/#19 use existing `components/ui/`, no bespoke styling. Own gated FORGE→ANVIL pass; on-device calibration on the V3 (label-learning already done).
- **📋 COMPULSORY BEEF-LABELLING REQUIREMENTS (RPA email, Daniel Hamilton, 2026-06-30 — authoritative; link: gov.uk "Comply with the Compulsory Beef Labelling Scheme"):**
  - **Intake/receiving:** allocate a unique batch number; record origin/born&reared, slaughtered-in site (country+plant, e.g. `GB1234`), cut-in site (country+plant, e.g. `GB5678`), and the allocated batch. **Mixed-origin deliveries** (e.g. some boxes born IE, some UK, or different slaughterhouses) must be booked in SEPARATELY each with its own batch number.
  - **Beef on site:** must always carry at minimum a batch number traceable to intake records (which hold the full compulsory indications). Lost traceability ⇒ out of the human food chain or made into a non-BLS product.
  - **Dispatch MINCE label:** Origin/born&reared · "Slaughtered in GB" (country) · batch unique to one day's production · "Minced in GB".
  - **Dispatch NON-MINCE/PREP label (e.g. topside):** Origin/born&reared · "Slaughtered in GB1234" (site+country) · "Cut in GB5678" (primary cut site+country) · your cutting details "Further cut in GB2946" · batch unique to one day's production.
  - **Compulsory WORDING (use verbatim where appropriate):** Slaughtered in · Cut in / Further cut in · Origin · Born in · Reared in · Minced in.
  - **Processed beef (e.g. kebabs):** if state altered ≥1% by additives/cooking, BLS labelling is NOT required — business decision whether to label anyway.
  - **Veal:** same as beef PLUS an age-category code in intake records AND final labels — `CAT V` (0–8 months), `CAT Z` (8–12 months).
- **⏸ DEFERRED follow-on units (after the beef mince+prep label, per Hakan 2026-06-30 "the rest can follow"):**
  - ~~**Veal age-category** (`CAT V`/`CAT Z`)~~ — **❌ DROPPED 2026-06-30 (Hakan): MFS does not sell veal, so the veal age-category requirement is N/A. Not a gap.**
  - ~~**Mixed-origin split at intake**~~ — **✅ NOT A GAP (verified 2026-06-30, evidence-based code audit). Hakan was right.** The goods-in intake form holds ONE origin set per submission (`app/haccp/delivery/page.tsx:1043,1070-1073`) and the server mints a fresh unique batch per row (`app/api/haccp/delivery/route.ts:92,129`). Booking a mixed-origin consignment as two separate deliveries = two batches is fully achievable TODAY → satisfies the RPA "book separately, each own batch" rule. It's a staff-training instruction, NOT a build. No code change.
  - ~~**MINCE dispatch label omits "Reared in"**~~ — **❌ CANCELLED 2026-06-30: NOT a requirement.** Authoritative gov.uk "Comply with the Compulsory Beef Labelling Scheme" (https://www.gov.uk/guidance/comply-with-the-compulsory-beef-labelling-scheme) confirms MINCE does NOT need separate "Born in"/"Reared in" lines — it needs **"Origin: [country]"** + "Slaughtered in [country]" + "Minced in [country]" + a traceability batch ref. So adding "Reared in" to mince would be wrong. The only mince WORDING nuance: our mince label prints "**Born in:**" where the scheme prescribes the literal "**Origin:**" (same info; one-word relabel on the mince template only). **Hakan 2026-06-30: leave mince untouched** unless he later wants the one-word fix. (NON-mince/PREP is different — it DOES require separate Born in + Reared in, which our prep label already does correctly: `route.ts:343-349`.)
  - **Processed-products exemption** (kebabs ≥1% altered) — ⚪ NOT a gap: BLS labelling is OPTIONAL for ≥1%-altered processed beef; business decision whether to label anyway. No code needed unless Hakan wants opt-out labelling.
  - **🆕 OPERATIONAL RULE worth knowing (gov.uk, not currently enforced in code):** a batch of MINCE must originate from a **SINGLE country of slaughter** — you must NOT mince beef from two different slaughter countries into one batch, and the mincer must be cleaned between different-country batches. Today the label aggregator comma-joins multiple slaughter countries for mince (`route.ts:262-270`), i.e. it would silently print "Slaughtered in GB,AU" if a mince batch's sources spanned two countries — which the scheme forbids. Likely a non-issue in practice (MFS mince is single-country), but the system does NOT warn. Decide: operational/training control vs a coded guard at mince intake. Surface to Hakan.
  - **✅ Country CODES vs full names — RESOLVED 2026-06-30.** The inspector's attachment ("shortened country codes" per Hakan) IS the code list he expects MFS to use → codes (GB, AU) on the labels are accepted by MFS's own enforcing officer. (The general gov.uk page leans toward full country names, but the site's actual inspector provided the code list, and his explicit guidance governs.) No change needed.
  - **Full beef-labelling status (2026-06-30, source-backed — gov.uk scheme page + inspector email + country-code attachment):** Intake capture + batch numbering + traceability ✓; mixed-origin via separate bookings ✓; PREP dispatch label (Born in / Reared in / Slaughtered in site / Cut in site / Further cut in GB2946) ✓; MINCE dispatch label ✓ except the "Born in:"→"Origin:" wording nuance (Hakan: leave it). Remaining genuinely-open items: the mince single-country-of-slaughter operational rule above; and Hakan's call on whether to do the one-word mince "Origin:" relabel.
...
- **🔵 Pass 3 follow-up (deferred — code-critic, PR #105):** `generateBarcodeSVG` (`lib/printing/html.ts`) embeds `batch_code` raw into the inline-SVG `<text>` node in the delivery print iframe — Pass 3 escaped the visible `bc` div copy + all other DB free-text but NOT this barcode-caption copy. Low risk today (`batch_code` is system-generated, e.g. `3006-GB-1`, not user-typed); inline-SVG `<script>` executes, so **promote to 🔴 if `batch_number` ever becomes user-editable**. One-line fix: `escapeHtml(text)` inside `generateBarcodeSVG`.
