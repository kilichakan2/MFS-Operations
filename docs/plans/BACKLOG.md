...
- **⏸ DEFERRED follow-on units (after the beef mince+prep label, per Hakan 2026-06-30 "the rest can follow"):**
  - **Veal age-category** (`CAT V`/`CAT Z`) — NEW staff-entry field at intake + on the label ⇒ app-screen change (add via existing `components/ui/` per #19; new DB column on `haccp_deliveries`).
  - **Mixed-origin split at intake** — currently one delivery = one batch (no split logic; scalar `born_in`/`slaughter_site`/`cut_site`). Decide operational (staff book two deliveries) vs coded multi-batch split.
  - **Processed-products exemption** (kebabs ≥1% altered) — business decision + optional opt-out labelling.
  - **🔴 Full beef-labelling regulatory review** — audit ALL labels against UK/FSA BLS + the inspector's attached documentation (NOT yet received — request the PDF) once the mince+prep label lands.
