...
- **Owner unit (next):** F-PROD-04 Pass 2b = build a proper RELEASE-SIGNED APK (keystore + signingConfig in `android/app/build.gradle` + versionCode bump + turn OFF `setWebContentsDebuggingEnabled`), proven on the physical Sunmi V3 over USB; distribution = manual sideload + Google Play if easy. Then Pass 3 = real allergens on the delivery label. Both their own gated FORGE→ANVIL passes.
