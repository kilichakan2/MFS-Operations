...
describe("isValidStatus", () => {
  ...
  it("arbitrary string is invalid", () =>
    expect(isValidStatus("yes")).toBe(false));
...
describe("isChecklistComplete", () => {
  function makeComplete(): Checklist {
    ...
    for (const s of REVIEW_SECTIONS) {
      cl[s.key].items = cl[s.key].items.map((item) => ({
        ...item,
        status: "ok" as const,
      }));
    }
...
describe("isValidReviewPeriod", () => {
  ...
  const future = new Date(Date.now() + 86400000 * 10).toLocaleDateString(
    "en-CA",
  );
...
describe("REVIEW_SECTIONS — Section 3.2 Training", () => {
  ...
  it("section 3.2 title is Training", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.2")?.title).toBe(
      "Training",
    );
  });
  ...
  it("section 3.2 has data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.2")?.hasDataPanel).toBe(
      true,
    );
  });
...
describe("trainingRefreshStatus", () => {
  ...
  function daysFromToday(n: number): string {
    ...
    // Local date (en-CA → YYYY-MM-DD), matching the implementation's
    // local-midnight basis. toISOString() returns the UTC date, which
    // disagrees with local for one hour after midnight in +offset zones
    // and made these boundary assertions flake 00:00–01:00 local (F-TD-13).
    return d.toLocaleDateString("en-CA");
  }

  it("null → not_recorded", () =>
    expect(trainingRefreshStatus(null)).toBe("not_recorded"));
  it("undefined → not_recorded", () =>
    expect(trainingRefreshStatus(undefined)).toBe("not_recorded"));
  it("empty string → not_recorded", () =>
    expect(trainingRefreshStatus("")).toBe("not_recorded"));

  it("yesterday → overdue", () =>
    expect(trainingRefreshStatus(daysFromToday(-1))).toBe("overdue"));
  it("30 days ago → overdue", () =>
    expect(trainingRefreshStatus(daysFromToday(-30))).toBe("overdue"));
  it("365 days ago → overdue", () =>
    expect(trainingRefreshStatus(daysFromToday(-365))).toBe("overdue"));

  it("today → due_soon (0 days = within 90)", () =>
    expect(trainingRefreshStatus(daysFromToday(0))).toBe("due_soon"));
  it("45 days → due_soon", () =>
    expect(trainingRefreshStatus(daysFromToday(45))).toBe("due_soon"));
  it("90 days → due_soon", () =>
    expect(trainingRefreshStatus(daysFromToday(90))).toBe("due_soon"));

  it("91 days → current", () =>
    expect(trainingRefreshStatus(daysFromToday(91))).toBe("current"));
  it("180 days → current", () =>
    expect(trainingRefreshStatus(daysFromToday(180))).toBe("current"));
  it("365 days → current", () =>
    expect(trainingRefreshStatus(daysFromToday(365))).toBe("current"));
});
...
describe("REVIEW_SECTIONS — Section 3.3 Personal Hygiene & Health", () => {
  ...
  it("section 3.3 title is correct", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.3")?.title).toBe(
  ...
  it("section 3.3 has data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.3")?.hasDataPanel).toBe(
      true,
    );
  });
...
describe("Section 3.3 data panel empty states", () => {
  it("all empty arrays produce totalRecords = 0", () => {
    ...
    const total =
      data.new_staff.length + data.exclusions.length + data.visitors.length;
    expect(total).toBe(0);
  });
...
describe("REVIEW_SECTIONS — Section 3.4 Cleaning & Disinfection", () => {
  ...
  it("section 3.4 title is correct", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.4")?.title).toBe(
  ...
  it("section 3.4 has data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.4")?.hasDataPanel).toBe(
      true,
    );
  });
  ...
  it("buildInitialChecklist includes 3.4 with 4 items and null statuses", () => {
    ...
    expect(cl["3.4"].items[0].label).toBe(
...
describe("Cleaning data panel logic", () => {
  ...
  it("low_temp_list: only records where sanitiser_temp_c < 82 (strict)", () => {
    ...
    const lowTemps = records.filter(
      (r) => r.sanitiser_temp_c !== null && r.sanitiser_temp_c < 82,
    );
...
describe("REVIEW_SECTIONS — Section 3.5 Pest Control", () => {
  ...
  it("section 3.5 title is correct", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.5")?.title).toBe(
      "Pest Control",
    );
  });
  ...
  it("section 3.5 has NO data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.5")?.hasDataPanel).toBe(
      false,
    );
  });
  ...
  it("buildInitialChecklist includes 3.5 with 6 items and null statuses", () => {
    ...
    expect(cl["3.5"].items[0].label).toBe(
      "Pest control contract in place and service contract reviewed",
    );
...
describe("REVIEW_SECTIONS — Section 3.6 Temperature Control", () => {
  ...
  it("section 3.6 title is correct", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.6")?.title).toBe(
      "Temperature Control",
    );
  });
  ...
  it("section 3.6 has data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.6")?.hasDataPanel).toBe(
      true,
    );
  });
...
describe("Temperature data panel logic", () => {
  const makeCalibManual = (pass: boolean, daysAgo: number, id = "Probe 1") => {
    ...
    return {
      ...
      cert_reference: null,
      ice_water_result_c: pass ? 0.5 : 2.5,
      ice_water_pass: pass,
      boiling_water_result_c: pass ? 99.5 : 97,
      boiling_water_pass: pass,
    };
  };
  const makeCalibCert = (daysAgo: number, ref = "CERT-001") => {
    ...
    d.setDate(d.getDate() - daysAgo);
    return {
      thermometer_id: "Certified",
      calibration_mode: "certified_probe",
      date: d.toISOString().slice(0, 10),
      cert_reference: ref,
      ice_water_result_c: null,
      ice_water_pass: null,
      boiling_water_result_c: null,
      boiling_water_pass: null,
    };
  };
  ...
  it("stale calibration: > 31 days → overdue flag", () => {
    ...
    const days = Math.floor(
      (today.getTime() - new Date(r.date).getTime()) / 86_400_000,
    );
  ...
  it("cold storage: non-pass status → alert", () => {
    const units = [
      { latest: { temp_status: "amber" } },
      { latest: { temp_status: "pass" } },
    ];
    const fails = units.filter(
      (u) => u.latest && u.latest.temp_status !== "pass",
    );
  ...
  it("delivery temp_cas uses temp_status not corrective_action_required", () => {
    ...
    const temp_cas_wrong = all.filter(
      (d) => d.corrective_action_required,
    ).length;
...
describe("REVIEW_SECTIONS — Section 3.7 Supplier Control & Traceability", () => {
  ...
  it("section 3.7 title is correct", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.7")?.title).toBe(
  ...
  it("section 3.7 has data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.7")?.hasDataPanel).toBe(
      true,
    );
  });
...
describe("Supplier data panel logic", () => {
  ...
  it("BLS completeness: all complete → no alert", () => {
    const g = {
      ...
      has_batch: 17,
      meat_total: 13,
      meat_bls_complete: 13,
    };
    const blsIncomplete =
      g.meat_total > 0 && g.meat_bls_complete < g.meat_total;
  ...
  it("hasAlerts false when everything clean", () => {
    ...
    const blsIncomplete =
      g.meat_total > 0 && g.meat_bls_complete < g.meat_total;
    const hasAlerts =
      s.total - s.formally_approved > 0 ||
      s.expired_certs > 0 ||
      s.expiring_60_days > 0 ||
      blsIncomplete ||
      sp.review_due > 0;
    expect(hasAlerts).toBe(false);
  });
...
describe("REVIEW_SECTIONS — Section 3.8 Incidents & Complaints", () => {
  ...
  it("section 3.8 title is correct", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.8")?.title).toBe(
  ...
  it("section 3.8 has data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.8")?.hasDataPanel).toBe(
      true,
    );
  });
...
describe("REVIEW_SECTIONS — Section 3.9 Food Fraud & Food Defence", () => {
  ...
  it("section 3.9 title is correct", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.9")?.title).toBe(
  ...
  it("section 3.9 has data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.9")?.hasDataPanel).toBe(
      true,
    );
  });
...
describe("FoodFraudDefencePanel logic", () => {
  ...
  it("hasAlerts false when both current", () => {
    ...
    const hasAlerts =
      ff.review_due || !ff.exists || fd.review_due || !fd.exists;
    expect(hasAlerts).toBe(false);
  });
...
describe("REVIEW_SECTIONS — Section 3.10 Premises & Equipment", () => {
  ...
  it("section 3.10 title is correct", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.10")?.title).toBe(
      "Premises & Equipment",
    );
  });

  it("section 3.10 has exactly 4 items", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.10")?.items).toHaveLength(
      4,
    );
  });

  it("section 3.10 has no data panel", () => {
    expect(REVIEW_SECTIONS.find((s) => s.key === "3.10")?.hasDataPanel).toBe(
      false,
    );
  });
