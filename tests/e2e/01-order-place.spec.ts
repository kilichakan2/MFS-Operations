...
test.describe("@critical order placement flow", () => {
  test("sales rep places an order with catalogued + ad-hoc lines", async ({
    page,
  }) => {
  ...
  test("dashboard shows the new order under Today + tomorrow", async ({
    page,
  }) => {
    await loginAs(page, "sales");
    ...
    await expect(page.getByText(/MFS-\d{4}-\d{4}/).first()).toBeVisible({
      timeout: 10_000,
    });
  });
});
