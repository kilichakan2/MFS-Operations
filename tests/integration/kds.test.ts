...
describe('KDS integration', () => {
  ...
  beforeAll(async () => {
    ...
    await supa.from('users').update({ pin_hash: hash }).eq('id', users.butcher.id)
  ...
  async function createAndPrintOrder(lineCount = 2): Promise<string> {
    const lines = Array.from({ length: lineCount }, (_, i) =>
      i === 0
        ...
        : { ad_hoc_description: `line ${i + 1}`, quantity: 1, uom: 'unit' as const }
    )
  ...
  it('GET /api/kds/orders returns printed orders without auth', async () => {
    ...
    const body = res.body as { orders: Array<{ id: string; state: string }>; recent_flashes: unknown[] }
    expect(body.orders.some(o => o.id === orderId)).toBe(true)
    expect(body.orders.find(o => o.id === orderId)?.state).toBe('printed')
  })
  ...
  it('marks a line done and auto-completes the order when last line done', async () => {
    ...
    const supa    = getServiceClient()
    const { data: lines } = await supa.from('order_lines').select('id').eq('order_id', orderId).order('line_number')
    ...
    const mid = await supa.from('orders').select('state').eq('id', orderId).single()
    ...
    const fin = await supa.from('orders').select('state, completed_at').eq('id', orderId).single()
