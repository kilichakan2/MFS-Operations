...
describe("/api/orders/[id]/picking-list integration", () => {
  ...
  it("rejects POST from sales role — only office/warehouse/admin can print (403)", async () => {
  ...
  it("returns HTML and transitions placed → printed", async () => {
    ...
    const { data } = await supa
      .from("orders")
      .select("state, printed_at, printed_by")
      .eq("id", id)
      .single();
  ...
  it("rejects reprint of a completed order (409 — was 403 pre-F-08, plan §10.5)", async () => {
    const id = await createPlacedOrder();
    ...
    expect((reprint.body as { code: string }).code).toBe("CONFLICT");
  });
