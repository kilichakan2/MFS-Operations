...
import {
  ...
  EmptyState, RowHead, TableRow, ListRow, StagePill,
} from '@/app/dashboard/admin/_components/primitives'
import { pickStageColor } from '@/app/dashboard/admin/_components/cards'

// ─── Types (mirrors /api/admin/prospects response shape) ─────────────────────
...
export default function AdminProspectsPage() {
  return (
    <Suspense fallback={null}>
      <AdminProspectsPageBody />
    </Suspense>
  )
}

function AdminProspectsPageBody() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setRows((data.rows ?? []) as ProspectRow[])
  ...
  return (
    <div className="min-h-screen bg-mfs-soft-neutral">
      <AppHeader title="Prospects" maxWidth="4xl"
        actions={
      ...
      <main className="max-w-5xl mx-auto px-4 py-5 pb-24 space-y-5 md:space-y-6">
        ...
        <PageHeading>Admin · Prospects</PageHeading>

        <Card>
          <CardHead title="Prospects this week" count={rows.length} />
          {rows.length === 0 ? <EmptyState rangeLabel="this week" /> : (
            <>
              {/* Desktop table — value column dropped (no schema source) */}
              <div className="hidden md:block">
                <RowHead
                  cols={['Account', 'Postcode', 'Stage', 'Visit type', 'Outcome', 'Rep']}
                  ...
                  <ListRow
                    ...
                    cells={
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-mfs-neutral-500 mt-0.5 capitalize">
                          {p.postcode || '—'} · {p.visitType || '—'} · {p.outcome || '—'}
                        </div>
                      </div>
                    }
                  />
