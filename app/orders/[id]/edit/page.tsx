...
function EditOrderPageInner({ params }: EditOrderPageProps) {
  ...
  const productItems: SelectableItem[] = products.map((p) => ({
    ...
    label: p.name,
    sublabel: p.box_size
      ? `${p.box_size}${p.category ? ` · ${p.category}` : ""}`
  ...
  const [productPickerForLineKey, setProductPickerForLineKey] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        if (!res.ok) {
          setLoadError(body?.message ?? `Server error (${res.status})`);
          setLoading(false);
          return;
        }
        ...
        setOrder(o);
        if (o.customer)
          setCustomer({ id: o.customer.id, label: o.customer.name });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const body = await res.json().catch(() => ({}));
      if (!res.ok) {
        // F-08 envelope: prefer per-field messages (e.g. each unknown
        // product named separately), then the structured message.
        setSubmitError(
          body?.fields
  ...
  const submitDisabled =
    submitting || isCompleted || (isPrinted && !canStillEdit);
    ...
    <>
                    ...
                    <div>
                      <div className="flex gap-2 mb-2">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            updateLine(line.key, { product_id: null })
                          }
                          disabled={submitDisabled}
                          className={`flex-1 px-3 py-2 rounded-lg text-xs font-bold border-2 transition-colors disabled:opacity-50 ${
                            !line.product_id && line.ad_hoc !== ""
                      ...
                      {line.product_id ? (
                        ...
                        <input
                          ...
                          value={line.ad_hoc}
                          onChange={(e) =>
                            updateLine(line.key, { ad_hoc: e.target.value })
                          }
                          disabled={submitDisabled}
      ...
      {productPickerForLineKey && (
        <BottomSheetSelector
          ...
          selectedId={
            lines.find((l) => l.key === productPickerForLineKey)?.product_id ??
            undefined
          }
        />
      )}
