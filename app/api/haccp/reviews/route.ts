    ...
    if (type === 'weekly') {
      ...
      if (problems.length > 0 && inserted) {
        const caRows = problems.map((i) => ({
          ...
          action_taken:  i.action?.trim() || `No action notes recorded at time of review — refer to weekly review record (week ending ${week_ending})`,
    ...
    if (type === 'monthly') {
      ...
      if (sysProblems.length > 0 && inserted) {
        const caRows = sysProblems.map((i) => ({
          ...
          action_taken:  i.notes?.trim() || `No action notes recorded at time of review — refer to monthly review record (${month_year})`,
          product_disposition:   'assess',
